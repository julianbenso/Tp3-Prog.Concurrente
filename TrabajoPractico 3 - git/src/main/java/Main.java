...
public class Main {
	...
	private final static int SERVICE_RATE_1 = 1;
	private final static int SERVICE_RATE_2 = 1;
...
//		procD2.interrupt();
		...
		while(!(finalizador.finPrograma2())) {
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
				e.printStackTrace();
		...
		String duracion = "Duracion del programa: "+ timeElapsed.toMinutes() +" minutos " + timeElapsed.minusMinutes(timeElapsed.toMinutes()).getSeconds()+ " segundos.";
